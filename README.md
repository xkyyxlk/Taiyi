...
长期开发以 [项目迭代工作流](docs/项目迭代工作流.md) 为准。当前断点记录在
[迭代状态](docs/迭代状态.md)，切换 Codex 会话时可以直接使用
[会话交接模板](docs/会话交接模板.md)。每次迭代的目标、实际改动、决策、验证和提交
永久保存在[迭代记录索引](docs/迭代记录/记录索引.md)。新会话必须先读取这些文件并
核对 Git 现场，不得只依赖旧会话记录。
